<script lang="ts" context="module">
  ...
  export const getRadius = (bounds: BoundingBox) => ({
    gtRadius: (r: Radius) => Math.floor((Math.min(bounds.w, bounds.h) / 2) * r),
  });

  export const getPivot = (bounds: BoundingBox): Coords => [Math.floor(bounds.w / 2), Math.floor(bounds.h / 2)];

  export const toRel = (boundsExt: BoundingBoxExt) => ({
    ttRel: (x: XCoord, y: YCoord): Coords => {
